...
async function sendRcon(command, { silent = false } = {}) {
  ...
  try {
    const res = await apiFetch('/api/command', { method: 'POST', body: JSON.stringify({ command }) });
...
function connectEventStream() {
  ...
  const url = key ? `/api/events?apiKey=${encodeURIComponent(key)}` : '/api/events';
  const es = new EventSource(url);

  es.onopen = () => {
    state.wsConnected = true;
    elements.wsPill.classList.add('connected');
    elements.bridgePill?.classList.add('connected');
    elements.rconPill.classList.add('connected');
  };

  es.onerror = () => {
    state.wsConnected = false;
    elements.wsPill.classList.remove('connected');
    es.close();
  ...
  es.onmessage = (event) => {
    ...
    if (data.message) {
      const bridgeType = data.type;
      const line = `[${data.timeFormatted || ''}] [${bridgeType}] ${data.message}`;
      const type = bridgeType === 'chat' ? 'chat' : bridgeType === 'join' || bridgeType === 'quit' ? 'player' : bridgeType === 'death' ? 'warn' : '';
      appendLogLine(line, type);
      if (bridgeType === 'join' || bridgeType === 'quit') {
        const player = data.data?.player || data.message.split(' ')[0];
        showToast(`${player} ${bridgeType === 'join' ? 'joined' : 'left'}`, bridgeType === 'join' ? 'success' : 'info');
        playSound(bridgeType);
        refreshStatus();
      }
    }
  };
...
async function init() {
  ...
  const configRes = await fetch('/api/config', { credentials: 'include' }).catch(() => null);
  if (!configRes?.ok) { showLogin(); return; }
